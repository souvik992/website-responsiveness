...
- **Functional Test Suite** (`.github/workflows/functional-test-suite.yml`) — runs both automatically and on demand. Unlike the two matrix workflows above, this one genuinely fails the job on a real assertion failure. Automatic staging (via `--grep` against each test's `@ID` tag, filtering `test-cases/testCaseDefinitions.ts`'s entries — no duplicated spec files):
  - **Push to any branch except `main`** → *Quick Smoke Tests* (SAN-001..005, ~5 min, no real order).
  - **Pull request targeting `main`** → *Critical Regression* (every P0 case: SAN-001..006, E2E-001, E2E-004, API-001..004, ~15-20 min, 4 real orders).
  - **Push to `main`** (i.e. a merge) → *Full Regression* (all 17 cases, ~12-45 min, 8 real orders).
  - **`workflow_dispatch`** → pick any suite by hand (sanity/e2e/api/all), same as before.

  A run is superseded (and cancelled) by a newer run on the same branch, so pushing several commits in quick succession doesn't stack up multiple real-order runs. Each stage produces one `functional-test-report-<label>` artifact containing the xlsx test-case tracker plus the generated Allure HTML report.
